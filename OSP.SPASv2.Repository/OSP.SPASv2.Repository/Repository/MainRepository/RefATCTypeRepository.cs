...
{
    public class RefATCTypeRepository : IRefATCTypeRepository<RefATCType>
    {

        #region Private Member Variables

        private SPASv2Context _context;
        AbstractRepository<RefATCType> _AbstractRepository;
        StringBuilder sb;
        ...
        #region Constructors
        public RefATCTypeRepository(SPASv2Context context)
        {
            _context = context;
            _AbstractRepository = new AbstractRepository<RefATCType>(_context);
        } 
        public async Task<IList<RefATCType>> GetATCTypeList()
        {
            try
            {
                IList<RefATCType> vlist = await _context.RefATCType.FromSqlRaw("select * from RefATCType").ToListAsync();
