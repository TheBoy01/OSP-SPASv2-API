...
{
    public class RefCityRepository : IRefCityRepository<RefCity>
    { 

        #region Private Member Variables

        private SPASv2Context _context;
        AbstractRepository<RefCity> _AbstractRepository;
        ...
        #region Contructor
        public RefCityRepository(SPASv2Context context)
        {
            _context = context;
            _AbstractRepository = new AbstractRepository<RefCity>(_context);
        }
        ...
        public async Task<IList<RefCity>> GetCityList()
        {
            try
            {
                ...
                IList<RefCity> vlist = await _context.RefCity.FromSqlRaw("select * from RefCity where Active=1").ToListAsync();
        ...
        public async Task<IList<RefCity>> GetCityByProvinceCode(string provinceCode)
        {
            ...
            {
                //var vlist =   _context.qryBranch.FromSqlRaw("exec sp_getbranchdetails '" + companycode + "','" + branchcode + "'").AsEnumerable().FirstOrDefault();
                IList<RefCity> vlist = await _context.RefCity.FromSqlRaw("select * from RefCity where Active=1 and ProvinceCode='" + provinceCode + "'").ToListAsync();
