...
namespace OSP.SPASv2.Repository.Utility
{
    public static class UtilitiesHttpClient<TEntity> where TEntity : class
    {
            ...
            try
            {
                using (var client = new HttpClient())
                {
                    ...
                    else
                    {
                        ...
                        //throw new Exception($"Failed to fetch data. Status code: {response.StatusCode}"+ "POST ASYNC T" );
                    }
            ...
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    else
                    {
                        // Handle error cases here, throw exception or return default value based on your use case
                        // For example, you can throw an exception with the error message
                        //return CreateErrorResult<T>($"Error: {response.StatusCode} - {response.ReasonPhrase}");
                        ...
                        //{
                        //    Status = "FAILED",
                        //    ErrorMessage = $"Error: {response.StatusCode} - {response.ReasonPhrase} POST ASYNC T"
                        //};
        ...
        private static T CreateErrorResult<T>(string errorMessage) where T : new()
        {
            T errorResult = new T();
            ...
            {
                statusProperty.SetValue(errorResult, "FAILED");
            }

            var errorMessageProperty = typeof(T).GetProperty("ErrorMessage");
            if (errorMessageProperty != null && errorMessageProperty.PropertyType == typeof(string))
            {
                errorMessageProperty.SetValue(errorResult, errorMessage);
            }
